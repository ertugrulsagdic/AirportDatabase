...
insert into Manager values(1, 'Technic', '12345678')
insert into Manager values(2, 'Technic', '12345678')
insert into Manager values(3, 'Technic', '12345678')
insert into Manager values(4, 'Traffic', '12345678')
insert into Manager values(5, 'Traffic', '12345678')
insert into Manager values(6, 'Traffic', '12345678')
insert into Manager values(7, 'Emergency', '12345678')
insert into Manager values(8, 'Emergency', '12345678')
insert into Manager values(9, 'Emergency', '12345678')
insert into Manager values(10, 'Management', '12345678')
...
DECLARE
	...
	@leavingTime time,
	@price float
set @arrivalTime = (SELECT ArrivalTime   
FROM Car_Park CP INNER JOIN Car C ON CP.SpaceNumber = C.CarSpaceNumber
WHERE C.PlateNumber = @PlateNumber)
set @leavingTime = (SELECT LeavingTime   
FROM  Car_Park CP INNER JOIN Car C ON CP.SpaceNumber = C.CarSpaceNumber
WHERE C.PlateNumber = @PlateNumber)
set @diff = DATEDIFF(HOUR, @arrivalTime, @leavingTime)
set @dailyPrice = (SELECT DailyPrice   
FROM Car_Park CP INNER JOIN Car C ON CP.SpaceNumber = C.CarSpaceNumber
WHERE C.PlateNumber = @PlateNumber)
set @price = (@dailyPrice / 24) * @Diff
select @price
update Car
set HourlyPrice = @price
where Car.CarSpaceNumber = (SELECT SpaceNumber   
FROM Car_Park CP INNER JOIN Car C ON CP.SpaceNumber = C.CarSpaceNumber
WHERE C.PlateNumber = @PlateNumber);
select Car.HourlyPrice from Car
end
execute CalculatePrice '01 OCK 1998'

drop procedure PersonInfo
...
FROM Manager INNER JOIN
	Employee ON Manager.ManagerEmployeeID = Employee.EmployeeID INNER JOIN
	Person ON Employee.EmployeeTCKN = Person.TCKN
	where Manager.ManagerEmployeeID = @managerID
	and Manager.ManagerEmployeeID = Employee.EmployeeID 
	and Employee.EmployeeTCKN = Person.TCKN

execute PersonInfo 1
...
begin
	select *
	from Airport_Store a where a.name = @name
end

execute SearchStore 'grange'
...
execute SearchSalary 1


create procedure PilotInfo
	@flightID varchar(7)
as
	SELECT * 
	FROM Pilot INNER JOIN Flight ON Pilot.PilotID = Flight.FPilotID INNER JOIN
	Person ON Pilot.PTCKN = Person.TCKN
	where Flight.FlightID = @flightID

execute PilotInfo 'THY2342'

drop procedure TicketInfo

create procedure TicketInfo
	@TCKN numeric(11)
	as
SELECT *        
FROM Passenger INNER JOIN
Flies ON Passenger.PassengerTCKN = Flies.PassengerTCKN INNER JOIN
Passenger_Flight ON Flies.PFlightID = Passenger_Flight.PFlightID INNER JOIN
Person ON Passenger.PassengerTCKN = Person.TCKN INNER JOIN
Ticket ON Passenger.PassengerTCKN = Ticket.TicketPassengerTCKN
where Passenger.PassengerTCKN = @TCKN

execute TicketInfo 12345678901
